...
f1::pp::output::ppline::output::output( const core::Iidentifier * ident ) noexcept:
	super( ident ),
	m_last_lineno( 0 ), m_was_nl( true ), m_was_space( true )
{
}

const status_t f1::pp::output::ppline::output::write( const char * string, size_t length )
{
	// Check current state
...
const status_t f1::pp::output::ppline::output::on_file_changed( const char * filename, f1::parser::lineno_t lineno ) __noexcept
{
	core::Tref<pp::Istorage> stg = m_parser->get_storage();
	__debugbreak_if( nullptr == stg )
		return STATUS_ERROR_NOT_INITIALIZED;
