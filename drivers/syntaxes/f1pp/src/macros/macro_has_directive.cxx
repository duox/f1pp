/*macro_has_directive.cxx*/
#include "../pch.h"
#include "../main.h"

const status_t f1::f1pp::macro_has_directive::initialize( const f1::pp::attribute_list * attrs,
...
const char * f1::f1pp::macro_has_directive::get_name() const
{
	return "__has_directive";
}

const status_t f1::f1pp::macro_has_directive::expand( pp::Iparser & parser, pp::string & buf, const pp::arg_list * args ) const
{
	status_t status;

	// Check current state
	__debugbreak_if( 1 != args->get_arg_list().size() )
		return parser.message( syntax::IMSG_ERROR_EXPECTED_S, "__has_directive( directive )" );
	...
	if( !ident.is_empty() )
	{
		...
		parser.find_directive( drct, pp::Iparser::fl_compare_no_case, ident );

		if( nullptr != drct && drct->get_flags( pp::Idirective::bf_disabled ) )
			drct = nullptr;

		buf.add_part_alloc( nullptr != drct ? "1" : "0" );
	}
...
/*END OF macro_has_directive.cxx*/
